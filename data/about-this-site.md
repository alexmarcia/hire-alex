...
## Architecture
This site is a retrieval augmented generation (RAG) chatbot with a Python backend built with LangChain and Anthropic's Claude, and a React frontend.
...
- Backend: a Python serverless function on Vercel (api/chat.py) that holds the API keys server side, enforces per IP rate limits, and caps message and history size.
- Frontend: React (Next.js) with a first person chat UI.
